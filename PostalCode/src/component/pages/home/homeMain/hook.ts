    ...
    const onFinish = (values: IWarehouseInqueryByPostalCode) => {
        ...
        if (apiMethod !== undefined) {
            apiMethod(data).then((response: any) => {
                if (response?.data?.ErrorMessage === null) { setResult(response?.data?.Data); setFullResult(response?.data?.Data); setShowTable(true) }
                else Notification.danger({ message: response?.data?.ErrorMessage})
            })
